...
if len(sys.argv) < 3:
	print("Usage: python3 generate.py <template_file> <parallel_class> [in_relation]\n") 
	sys.exit(1)
	
template_path = os.path.join(script_dir, sys.argv[1])
...
template = []
order = 10
parallel_class = int(sys.argv[2]) # 0 - 3
...
if __name__ == "__main__": 
	...
	with open(template_path, "r") as f:
		for line in f: 
			line = line.strip()
			line = [[int(char) for char in str(x)] for x in line.split()] # Converts line into list of variables
	...
	if parallel_class == 0:
		addCardinalityClauses([get1DIndex(r, 0) for r in range(order)], 1, 1)
		for r in range(order):
			for x in range(1, order):
				addImplicationClause([get1DIndex(r, x-1)], [get1DIndex(r, x)])
	elif parallel_class == 1:
		addCardinalityClauses([get1DIndex(0, c) for c in range(order)], 1, 1)
		for c in range(order):
			for x in range(1, order):
				addImplicationClause([get1DIndex(x-1, c)], [get1DIndex(x, c)])
	
	num_bits = len(template[0][0])
